class UserStocksController < ApplicationController
  ...
  def create
    if params[:stock_id].present?
      ...
      #this is a stock already found in the stock db, and just needs to be given a new user assoc
    else
      stock = Stock.find_by_ticker(params[:stock_ticker])
      #if not found by id, try search by ticker
      if stock 
        @user_stock = UserStock.new(user: current_user, stock: stock)
        #if found by ticker create user assoc
      else
        stock = Stock.new_from_lookup(params[:stock_ticker])
        #or look it up
        if stock.save
          @user_stock = UserStock.new(user: current_user, stock: stock)
          #and finally, save assoc.
        else
          @user_stock = nil
